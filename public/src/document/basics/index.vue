...
<template>
    <div id="document-page-basic">
        <div class="content_box">
            <h3>一、HTML 属性中的值应使用 v-bind 指令</h3>
            <h4>1、v-bind:title</h4>
            <div class="detail">
                <p v-bind:title="titleMsg">给元素绑定title属性</p>
                <p :title="titleMsg">v-bind缩写：即省略v-bind</p>
            </div>
            ...
            <div class="detail">
                <p>2.2绑定class中使用三元表达式判断</p>
                修改颜色<input type="checkbox" v-model="changeColor" />
                <span :class="changeColor?'red':'green'">给元素动态绑定class属性</span>
            </div>
            <div class="detail">
                <p>2.3绑定class中的数组</p>
                <span :class="['green', 'big']">给元素动态绑定class属性</span>
            </div>
            <h4>3、v-bind:href</h4>
            <div class="detail">
            ...
            <p v-cloak>{{titleMsg}}</p>
            <h4>10、v-once</h4>
            在第一次DOM时进行渲染，渲染完成后视为静态内容，跳出以后的渲染过程。
            <div class="detail border">
                <div v-once>第一次绑定的值：{{titleMsg}}</div>
                <div>双向绑定：{{titleMsg}}</div>
                <div><input type="text" v-model="titleMsg"></div>
            </div>
        </div>
        <div class="content_box">
            ...
            <h4>2、v-show, DOM已经加载, 通过CSS控制没有显示出来</h4>
            <div class="detail border">
        ...
        <div class="content_box">
            ...
            <h4>1、循环数组</h4>
            <div class="detail">
                <span v-bind:key="index" v-for="(name, index) in nameList">
                    {{index}}_{{name}}
            ...
            <h4>2、循环对象</h4>
            <div class="detail border">
                <span v-bind:key="index" v-for="(value, index) in personObject">{{value}}、</span>
                <br/>
                <span v-bind:key="index" v-for="(value, key, index) in personObject">{{index}}_{{key}}: {{value}}, </span>
            </div>
            <h4>3、循环数字</h4>
            <div class="detail">
                <span v-bind:key="index" v-for="(value, index) in 6">{{value}}、</span>
                <br/>
            </div>
        </div>
        <div class="content_box">
            ...
            <h4>2、v-on 还有一种简单的写法，就是用@代替</h4>
            <div class="detail">
                <p @click="clickMeDouble">
                    以2递增 {{count}}次
                </p>
            </div>
            <h4>3、绑定一个键盘回车事件, v-on:keyup.enter</h4>
            <div class="detail">
                <input type="text" @keyup.enter="clickMeDouble" v-model="count" />
            </div>
        </div>
        <div class="content_box">
        ...
        <div class="content_box">
            ...
            <div class="detail border">
                <h4>多个复选框</h4>
                <p v-bind:key="index" v-for="(name, index) in nameList">
            ...
            <div class="detail border">
                <h4>多行文本框</h4>
                <textarea cols="20" rows="5" v-model="titleMsg"></textarea>
            </div>
            <br/>
            <div class="detail border">
                <h4>下拉框</h4>
                <select v-model="selectedValue">
                    <option v-bind:key="index" v-for="(item, index) in selectOption" v-bind:value="item.value" >
                        {{item.label}}
                    </option>
                </select>
                <br/>
